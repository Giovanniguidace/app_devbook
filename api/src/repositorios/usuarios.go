...
// Atualizar realiza a alteração de informações de usuário no banco de dados
...
// BuscarPorEmail busca determinado usuário por email
...
func (repositorio Usuarios) BuscarSeguindo(usuarioID uint64) ([]modelos.Usuario, error) {
	linhas, erro := repositorio.db.Query(
		"select u.id, u.nome, u.nick, u.email, u.criadoEm from usuarios u inner join seguidores s on u.id = s.seguidor_id where s.seguidor_id = ?", usuarioID,
	)
	if erro != nil {
		return nil, erro
	}
...
// BuscarSenha busca a senha de um determinado usuário
...
// AtualizarSenha atualiza a senha de um determinado usuário no banco
func (repositorio Usuarios) AtualizarSenha(usuarioID uint64, senhaComHash string) error {
	...
	_, erro = statement.Exec(senhaComHash, usuarioID)
	if erro != nil {
		return erro
	}
